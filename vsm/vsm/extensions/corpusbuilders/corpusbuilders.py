...
def corpus_fromlist(ls, files_names, context_type='context'):
    """
    ...
    **Examples**
    ...
    #print corpus[0:500]
    #print files_names[0:199]
    #for file in files_names[0:3]:
    #   with open('/home/jredondo/Proyectos/Analisis_del_Discurso/src/topic-explorer/demo-data/corpus_propuestas/noaccent/'+file,'r') as f:
    #        print f.name ,len(f.read().split(' '))

    indices = np.cumsum([len(sbls) for sbls in ls])
    #metadata = ['{0}_{1}'.format(context_type, i)
    #            for i in xrange(len(indices))]
    metadata = sorted(files_names)
    md_type = np.array(metadata).dtype
